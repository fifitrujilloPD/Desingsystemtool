...
function toTokenLabel(colorName: string, scale: string): string {
  const secondaryGroups = new Set(["orange", "purple", "pink", "blue", "brown"]);
  const prefix = secondaryGroups.has(colorName.toLowerCase()) ? "color-secondary" : "color-primary";
  return `${prefix}-${scale}`;
}
              ...
              return (
                ...
                >
                  <div className={`flex flex-col items-center justify-center py-6 px-1 ${textColor}`}>
                    <span className="text-xs font-semibold opacity-90">{toTokenLabel(colorName, scale)}</span>
